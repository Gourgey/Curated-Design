...
**Not done:** the full 10-layer file split (tokens/reset/layout/nav/forms/cards/carousels/modules/a11y/utilities), retiring the `--sp-*`/`--cp-*`/`--hp-*`/`--pp-*`/`--srv-*` aliases, and reducing `body.page`-specificity reliance. Per an explicit decision, the first safe increment of this work is a physical-only split — partial files concatenated at build time in their *existing* relative order, so nothing is reordered relative to anything else and the cascade-risk this section describes doesn't apply. True by-topic regrouping (e.g. gathering every carousel rule, wherever it currently lives, into one place) is deliberately deferred past that, since it requires the same one-change-at-a-time, pixel-diff-verified treatment demonstrated above, at the scale of the whole file rather than 27 known selectors.
