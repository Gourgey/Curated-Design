...
| Breadcrumb / back navigation | `src/_includes/partials/backlink.njk` | CSS consolidated (P2.2 follow-up, July 2026) into `.backlink`/`.backlink--pill`/`.backlink--hero` in `assets/css-partials/15-backlink.css`. Markup consolidated (16 July 2026) into two Nunjucks macros, `pill(href, label)` and `hero(href, label)`, imported by all four call sites (`app-pages.njk`, `legal-pages.njk`, `apps/legal-pages.njk`, `projects/project-pages.njk`) — verified byte-identical generated HTML against the pre-change build. This one was safe to consolidate as a macro because it contains no async shortcode; contrast with the card component below. Service pages still have no back-navigation; unchanged, see below. |
...
## Update (16 July 2026, round 3): backlink markup consolidated; card markup consolidation genuinely blocked

The CSS-level consolidation above left the *markup* duplicated per template (bare `<a>` vs `<p class="backlink-row">`; three separate inline card blocks) — extracting that into one shared partial per component was the next step this round attempted.

- **Backlink:** done. See the shared-partials table above — a two-macro Nunjucks partial, verified byte-identical output.
- **Card:** attempted, reverted, genuinely blocked rather than just not-done. All three card call sites (`projects.njk` completed grid, `projects.njk` concepts grid, `index.njk` services grid) include the async `{% image %}` Eleventy shortcode. Extracting the shared markup into a macro made every `{% image %}` call inside it render nothing (empty output, no error). Switching to `{% set cardProject = project %} {% include "partials/card-project.njk" %}` inside the `{% for %}` loop instead avoided the empty-output problem but introduced a worse one: the loop's `{% set %}` variable was overwritten by later iterations before earlier iterations' async `{% image %}` work resolved, so cards rendered with another iteration's image and text (confirmed live — e.g. the "Styling" service card's markup appeared under `concept_design`, `bespoke_furnishing`, and `interior_architecture_design` as well). This is a data-correctness bug, not a visual nitpick, so the change was fully reverted rather than shipped. Do not re-attempt a macro or `{% set %} + {% include %}` consolidation of the card component without first solving the async-shortcode-in-loop ordering problem — one untested direction: pre-render each card's image markup into an array *before* the loop body, then interpolate the resulting strings inside the loop instead of calling `{% image %}` from inside a macro/include.

## What this pass did not do

Building the global footer and quote/testimonial block remains blocked on their content-side decisions (open footer-links decision, genuine testimonial content) — not attempted, and shouldn't be until those are resolved. See "Components that are genuinely blocked" above. The card markup consolidation is blocked for a different, technical reason — see the round 3 update immediately above.
