...
- **Phase 3 is well advanced.** P3.1 (global footer) landed first, then P3.3 (Work index restructured into Completed / Concepts sections) and P3.4 (case-study model gaps closed at the template/schema level — next-project link, caption and image-type capability). P3.2 (Services index) was initially not applicable, then reversed and built as a lightly-linked hub page (see the P3.2 section below and `docs/overhaul/DECISIONS.md`). **P3.5 was declined by the site owner (16 July 2026)** — see the P3.5 section below and `docs/overhaul/DECISIONS.md`; the homepage's current layout is intentional and should not be restructured again without being explicitly asked for, even under a generic "continue the build plan" instruction. **P3.6 (Studio) is resolved** via the site owner's recorded decisions. **P3.8 (unique metadata) is now written** for every page it applies to (Work, Services, all 6 service pages, Studio, Contact, the one indexable project, and the Curiosity Tracker app pages) — see P3.8 below. **P3.9 (redirects/clean URLs) is now fully implemented** (16 July 2026) — every public URL except `/apps/*` (deliberately unchanged) and the homepage now lives at a clean directory URL (`/work/`, `/services/`, `/studio/`, `/contact/`, and their nested detail pages), with a complete `public/_redirects` 301 map covering every old route. **P3.7 (baked-in cover text)** is resolved, not blocked: the Garden Restaurant hero swap is fixed; the six service cover images turned out to already have clean twins in the repo (each service's `cardImage`), but the site owner reviewed that and explicitly chose to keep the baked text as-is (17 July 2026) — a recorded preference, not a gap. Marylebone Terrace's hero (a third baked-text instance found in a later sweep, no clean twin exists) was explicitly deprioritized by the site owner ("Ignore the Marylebone terrace") and isn't tracked as open work.
- **Phase 5 is substantially underway (16–17 July 2026), skipping ahead of Phase 4** (which started before Phase 3's last few loose ends resolved, since most of its own tickets didn't depend on them). P5.1 (self-hosted, subsetted fonts), P5.2 (responsive image delivery), P5.3/P5.4 (render-blocking audit, single LCP strategy), P5.5 (technical SEO — share images, structured data, sitemap `lastmod`), P5.6 (security headers, fully verified for public pages), and P5.7 (Decap SRI hash and editorial workflow, partially verified) are all implemented — see each ticket below for exactly what was verified versus what still needs a live owner test. **P5.8 is moot** (the custom visual CMS was already retired, P0.3). **P5.9 needs a legal professional**, not implementation work. **P5.10** (Lighthouse quality budgets) has not been attempted — it needs a production-like deploy to measure meaningfully, not just a local build.
...
- **The six service cover images** (`assets/images/curated_services/page_brands/*_cover.webp`) each have "Curated Services / [Service Name]" baked in, confirmed by direct visual inspection of `concept_design_cover.webp`.
- **Correction (17 July 2026):** the claim above that "no clean version of these exists in the repo" was wrong — it was never actually true. Each service already has a second, text-free rendering of the exact same photo sitting in the repo as its `cardImage` (`assets/images/curated_services/[slug].webp`, used on the Services index and homepage), separate from the baked-text `coverImage` used on that service's own detail-page hero (`assets/images/curated_services/page_brands/[slug]_cover.webp`). Visually confirmed all six pairs are the same underlying photograph, text overlay aside. This was caught by the site owner, not found independently — the original investigation checked whether the *cover* image had a clean twin but didn't check whether the *card* image was that twin.
- **Site owner decision (17 July 2026): keep the baked text.** Asked directly, given clean versions exist: "I like the text on top of each one. Why should this change if the images can be resized." No image swap was made — the six `coverImage` files stay exactly as they are, text included. This is a recorded preference, not an oversight; don't swap these to the clean `cardImage` versions without being asked again. The original reported symptom (mobile-width cropping cutting off the baked text's edges) is a separate, still-open question from this preference — cropping legibility on small viewports hasn't been re-tested since this decision, since the decision was about the text-on-image treatment in general, not specifically about whether the current mobile crop keeps that text fully readable.
- **Marylebone Terrace's hero/card image** (`assets/images/projects/marylebone_terrace/marylebone_terrace_1.webp`) also has "Coming Soon" baked in, with no clean twin found anywhere in the repo. **Explicitly deprioritized by the site owner (17 July 2026): "Ignore the Marylebone terrace."** Not pursued further; not tracked as open work.
...
- **Curiosity Tracker** (`src/_data/apps.js`) — added `metaTitle`/`metaDescription` at the app level and `pageMeta` for its privacy/support/terms pages, using facts published on those pages. ProcureCore also had complete, unique metadata at this stage; its metadata and policy content were subsequently rewritten in July 2026 for the account-based Supabase release.
...
**Note:** the six service cover images' baked-in text has no bearing on metadata; see P3.7 for that thread, which is now resolved (the site owner chose to keep the text, not remove it).
...
- [ ] No essential wording remains baked into project/service artwork. Garden Restaurant's hero fixed (16 July 2026, P3.7). The six service cover images still have baked-in text, but as of 17 July 2026 that's a deliberate site-owner decision ("I like the text on top of each one") made with full knowledge that clean twins exist in the repo — not a blocker, and not to be changed without being asked again. Marylebone Terrace's hero (a third instance, no clean twin exists) was explicitly deprioritized by the site owner and isn't being pursued. Left unchecked because the literal criterion still isn't met, not because anything remains outstanding.
...
**Implementation note (17 July 2026) — reverted per site owner decision:** an earlier pass added `/services/` to the primary nav pill and footer, on the reasoning that Services (already in the sitemap and breadcrumb structured data) was missing from global navigation. Widening the floating nav pill to 4 items caused a real regression — it overlapped the homepage's own sticky in-page pill row across roughly 1280–1480px viewport widths — which was fixed with a geometry-derived CSS reservation (`padding-right: max(0px, calc(1530px - 100vw))` on `.pillmenu-wrap`). The site owner then asked for the nav addition itself to be reverted, not just the overlap fixed: "I didn't ask for the services to be added to the nav" / "remove the services nav item." Both the nav/footer addition and the CSS reservation it required have been fully reverted (`nav.njk`, `footer.njk`, `04-homepage.css` all back to their pre-17-July state) — confirmed programmatically back to the original ~2px gap at 1280px and true dead-centre alignment at wider widths. **Services is once again not reachable from the primary nav or footer — this is now a deliberate, recorded site-owner decision, not an oversight**, and should not be re-added without being explicitly asked for again (same standing rule as the P3.5 homepage-layout decision). Not separately re-verified: 200%-zoom/text-enlargement pass and long-title stress test for the header/footer specifically — see P4.9.
...
**Implementation note (17 July 2026):** audited the live homepage against this ticket's criteria rather than re-implementing it — the site owner declined a homepage content-sequence restructure under P3.5 and the current sequence is an intentional preference (see `docs/overhaul/DECISIONS.md`). The featured carousel is already a real linked, labelled component (each slide is an `<a>` to its project with a kicker/status/title overlay, not an anonymous image swap). The LCP element is deliberate — a single eager-loaded first-carousel-image, tuned in P5.10. CTA hierarchy is a single `.home-cta` link, not competing buttons. No further changes made.
...
**Implementation note (17 July 2026):** audited against this ticket's criteria; no changes needed. Completed work already renders in its own leading section ahead of a visually and structurally distinct concepts section (P3.3). Cards are whole-card `<a>` elements, so click/tap and keyboard activation are the same target — no separate hidden hit area. Cards carry title plus a kicker line, and concept cards additionally carry a status label. Image ratios are held consistent through the shared `card` image transform.
...
**Implementation note (17 July 2026):** every field the CMS schema (`admin/config.yml`) defines for a project is already rendered somewhere on the page (intro/lead/body/callout/extra sections/facts/gallery captions and image-type labels) — no orphaned "previously lost" field was found. Found and fixed a real deduplication bug instead: when a project omits `heroImage`, the template falls back to the first gallery image, but the gallery carousel loop still rendered that same image again as its own slide — a literal duplicate, not just a near-miss. Fixed by excluding the image actually used as the hero from the gallery list before building carousel slides (new `excludeImage` filter in `.eleventy.js`, applied in `project-pages.njk`), and recomputing `hasHeroCarousel`/`mediaCount` from the deduplicated list so a project left with only a duplicate "gallery" image now correctly renders as a single static image with no carousel controls, rather than a fake two-slide carousel. Verified in a live preview: a real 3-image project (Marylebone Lobby) shows 3 distinct slides labelled "1 of 3"/"2 of 3"/"3 of 3", and a coming-soon single-image project (Botanical Residence) renders one static image with no carousel role or controls. `npm run check` passes. Related/next-project navigation and the enquiry CTA were already functional from P3.4. Reading measure (project-lead/body column) sits inside the site's existing 1150px container at a 3:2 column split, keeping line length reasonable at all viewports — no separate max-width was added.
...
**Implementation note (17 July 2026):** the service detail template previously had only a "Back to Services" pill, not real parent navigation. Added a visible Home / Services / [service] breadcrumb (`breadcrumb` macro in `backlink.njk`, styled in `15-backlink.css`) that mirrors the `BreadcrumbList` structured data already emitted in `base.njk`. Enquiry forms and budget-language fields were already standardised (shared `service-enquiry` form partial, common `glance` fields). Replacing the mobile-cropped text artwork on the six service cover images is **not being done** — see P3.7: clean twins of these images exist in the repo, but the site owner reviewed that and explicitly chose to keep the baked text. This is a closed decision, not a blocker on this ticket.
...
**Implementation note (17 July 2026):** the Studio page had a full "Documents & tools" section — its own eyebrow, heading, and a four-item hairline list of legal/app links styled at the same visual weight as the rest of the editorial body. Removed it entirely (`about.njk`, plus the now-dead `about.documents` CMS field and `.studio-docs`/`.doc-list` CSS) rather than just shrinking it: the footer already carries the same Legal and App-support links, generated from the live `legal.pages`/`apps.items` collections rather than this hand-maintained duplicate, so removing it also closes a content-drift risk, not just a visual-weight one.

**Founder section added (17 July 2026):** the ticket's core ask — "introduce the person or studio behind the work with approved imagery and facts" — needed real facts from the site owner, so `docs/overhaul/STUDIO-BIO-INTERVIEW-BRIEF.md` was written and put to them directly. The interview confirmed: name and title (Josh Gourgey, Founder — kept deliberately simple, no team size stated either way), a background in residential developments across London, hospitality (hotels and restaurants), and international work particularly in the Middle East (years-of-experience and the previous employer both deliberately not named, per the site owner's preference), the real motivation for going into interior design (the range of the work, not a "passion for design" cliché), and a 10-step example workflow. Added a new `founder` object to `about.json` (eyebrow/heading/paragraphs, editable in Decap via a new field in `admin/config.yml`) and a matching `studio-founder` section in `about.njk`, styled to match the existing `studio-locations` block (same eyebrow/heading/body grid, `05-legacy-pages.css`). Voice is "we," matching the rest of the page, at the site owner's explicit request. The confirmed workflow was folded into the existing three-step Process section's copy rather than added as a separate list, and the process closing note now states plainly that it's a representative example, not a fixed formula — both per the site owner's direction. No photography was added: the interview confirmed none is ready yet, and the brief was explicit that empty sections should stay empty rather than be padded. Verified via `npm run check` and an independent Puppeteer screenshot capture (`tools/capture-references.js`) after the Claude Code browser preview tool produced corrupted/stale captures this session — DOM inspection (`getBoundingClientRect`, computed styles) had already confirmed the page was correct before that fallback capture confirmed it visually. This ticket is now done, aside from the photography the site owner said isn't ready yet.
...
**Implementation note (17 July 2026):** audited the contact form and its JS (`assets/js/contact-form.js`) against this ticket's criteria; no changes needed — the substance was already built (likely in P1.6). Fields are trimmed to name, email, project type, an explicitly optional budget field (labelled "· optional" and without the `required` attribute — required/optional state is conveyed both visually and to assistive tech via the native attribute, not just CSS), and a project outline. On a failed submission the code path never calls `form.reset()` — entered values survive — while the error message is shown, focused, and marked `role="alert"`. On success the form is hidden, a `aria-live="polite"` success card is shown and focused, matching the real `/thank-you/` page's copy for the no-JS fallback path. `npm run check`'s accessibility suite already exercises this via automated "enquiry-form interaction" smoke tests and passes.
...
**Implementation note (17 July 2026):** there was no 404 page anywhere in the project — not a template, not a Netlify redirect rule, nothing; an unmatched route would have fallen through to Netlify's generic default error page. Built `src/404.njk` (permalink `404.html`, which Netlify serves automatically for any unmatched route with no `netlify.toml` rule needed) using the same shell, `.thanks`/`.button` components, and cream contact-page theme as the existing thank-you page, with links to Work, Services, and Contact per the ticket. Verified in a live preview and confirmed `npm run check`'s generated-site and accessibility scans still pass (31 HTML pages now, up from 30). Privacy/terms/company-information pages (`legal-pages.njk`) and the apps index/detail pages were already in the shared `base.njk` shell at restrained, non-dominant visual weight — audited, no changes needed. `/thank-you/` already carries `noindex: true`.
...
**Implementation note (17 July 2026, partial):** spot-checked `document.documentElement.scrollWidth` against `window.innerWidth` at 320px, 768px, 1440px, and 1920px across the homepage, work index, a project detail page, services index, a service detail page, studio, contact, the new 404 page, and the apps index — zero horizontal overflow at any width on any of them. Reduced-motion is already respected in `navigation.js`, `scroll-effects.js`, and `contact-form.js` (checked via `prefers-reduced-motion`). Keyboard/focus behaviour for navigation, the carousel, and forms is already covered by `npm run check`'s automated Axe + interaction smoke tests, which pass.

**Follow-up pass (17 July 2026):** completed the remaining checks that don't require the site owner's own hardware. 200%-zoom was approximated by testing at 640px viewport width (roughly what a 1280px-wide desktop shows at 200% browser zoom) on the homepage, a project page, and contact — no overflow on any. Long-content stress-tested by temporarily swapping in an extremely long project title and an unusually long form-error message via DOM manipulation (not persisted to real content) at 375px — no overflow either. No-JS static audit: primary nav links and the contact form are real `<a href>`/`<form method="POST" action="/thank-you/">` HTML, so core navigation and enquiry submission both work without JavaScript.

Two genuine gaps turned up and were fixed the same day (17 July 2026):
- **Touch target too small:** the "Back to Work" hero backlink on project pages (`.backlink--hero`) had a 20px-tall click target — under the WCAG 2.2 AA 24×24px minimum. Fixed by adding `padding: 12px 0` (`15-backlink.css`), taking the tappable area to 44×125px without moving the visible text — `.project-topbar`'s `align-items: center` keeps the glyphs anchored at the same point the extra padding is added symmetrically around. The carousel prev/next controls (42×42px) were already fine.
- **Gallery unreachable without JS:** the project-detail hero gallery only ever showed its first image without JavaScript — `.project-hero-track` defaults to that slide (`overflow: hidden` on its parent, `transform: translateX(0)` at rest) and the prev/next buttons do nothing without `carousels.js`. Fixed with a `<noscript>` fallback in `project-pages.njk`: the deduplicated gallery images render as a plain, unpaginated stacked list (new `.project-noscript-gallery` styles in `09-project-redesign.css`) that only exists in the DOM when scripting is unavailable — browsers with JS enabled never parse `<noscript>` content, confirmed live (`document.querySelector('.project-noscript-gallery')` returns `null` with JS on). Core content (body text, facts, CTA) already worked without JS; now the rest of the gallery does too.

Both verified in a live preview and `npm run check` passes.

**Still not done:** spot checks on real physical mobile devices — that needs the site owner's own hardware or a device-testing service, not something verifiable from this environment.

### Phase 4 exit gate

- [ ] Every core page is approved at 390px, 768px, and 1440px. Not "approved" in the sign-off sense this bullet means; see the P4.9 note for what was spot-checked (320/640/768/1440/1920px including a 200%-zoom approximation, zero overflow on the sampled templates) versus what wasn't (a dedicated touch-input pass, real devices).
- [x] No page-level horizontal overflow is present at 320px or wider (17 July 2026) — verified on a representative sample of templates (homepage, work index, project detail, services index, service detail, studio, contact, 404, apps index), not literally every page.
- [ ] All essential tasks are possible with keyboard, touch, and without JavaScript where applicable. Keyboard/focus is covered by the automated Axe + interaction suite. No-JS is verified for primary nav, the contact form, and (as of 17 July 2026) the project gallery, which now has a `<noscript>` fallback. A dedicated touch-input pass (as opposed to touch-target sizing, which is fixed) hasn't been done.
- [x] Completed work and studio credibility appear before speculative content (17 July 2026). Completed-work ordering was done (P3.3/P4.3); studio credibility now has a real Founder section (P4.6) with facts confirmed directly by the site owner. Portrait/environmental photography is still absent — the site owner confirmed none is ready yet, and the ticket's own brief said to leave that empty rather than pad it, so this isn't blocking the checkbox.
- [ ] Form, carousel, menu, and CTA states are consistent across templates. Reasonably consistent by inspection (shared form/carousel/nav components), but not run through an explicit cross-template consistency pass.
- [ ] Visual-regression changes are reviewed and intentional. No visual-regression tooling exists in this repo; this hasn't been screenshot-diffed against a baseline.
...
**Implemented (16 July 2026).** Added a `640` intermediate width to both the shortcode's `HERO_WIDTHS` (`400, 640, 800, 1200, 1600, 2400`) and `CARD_WIDTHS` (`400, 640, 800, 1200`) in `.eleventy.js`, closing the 400→800 gap the ticket calls out directly.

Investigated the homepage carousel's "requests 800px for ~340px" complaint concretely rather than guessing: its container (`.dock-panel`, `width: min(1000px, calc(100% - 40px))`) has its own `16px`/`16px`/`24px` padding that the `sizes` attribute (`(min-width: 1040px) 1000px, calc(100vw - 40px)`) never accounted for — the real rendered image width is 32px narrower than what was declared on both branches. Fixed to `(min-width: 1040px) 968px, calc(100vw - 72px)` (the extra `32px` in the mobile branch covers both the container's own `calc(100% - 40px)` clamp and its `16px`-per-side padding). Verified live at a 375px/2x-DPR viewport (a fresh browser tab, to rule out the picture-element "sticky selection across viewport changes" behavior that initially gave a misleading reading in a reused tab): the carousel image now resolves to the new `640w` derivative instead of the `800w` one it would have picked before — confirms the fix reaches production behavior, not just the markup.

**Mobile background candidate:** generated a `640`-wide AVIF/WebP pair (`assets/images/background-640.*`, via `sharp` from the existing `2560` source to avoid compounding compression loss) and added it as the smallest `<source>` candidate in `base.njk`'s fixed `#bg` `<picture>` element, alongside the existing `1280`/`1920`/`2560` tiers — addressing the "selected a 1280px AVIF unnecessarily" finding by giving narrower/lower-DPR viewports an actual smaller option, without touching the existing tiers (purely additive, so higher-DPR mobile devices are unaffected). Verified in a fresh tab at 375px/2x DPR (750px effective need): the browser correctly resolves to `background-1280.avif` — the genuinely correct minimal-sufficient tier for that need, confirming the selection logic works as intended; the `640` tier is reached at lower effective needs (narrower viewports and/or 1x DPR).

**"Avoid passing unused originals into `_site`" — investigated, not acted on; deferred to P5.5.** The obvious target looked like `assets/images/projects/` and `assets/images/curated_services/` (7.7MB combined) — no template ever renders an `<img src>` pointing at these raw source files directly, only the `{% image %}` shortcode's generated `/img/*` derivatives. But `base.njk`'s `og:image`/`twitter:image` tags resolve to the *raw* `project.data.heroImage`/`service.data.coverImage`/`site.settings.shareImage` path, not a derivative — meaning these exact files are the real, currently-live share-image URLs that social crawlers (Facebook, X, Slack, iMessage) fetch directly. Excluding them from the passthrough copy would silently break every page's share preview. A properly-sized, purpose-built share-image derivative (P5.5's "validate a real 1200×630 share asset") is the correct fix for this, not a passthrough-copy exclusion; left as-is here rather than risk a real regression for a build-output-size win.

Verified with `npm run check` (full gate) and live-preview checks at both mobile (375px) and desktop widths — screenshots match prior appearance at both.
...
**16 July 2026: an assumption-based fix that real measurement later proved wrong — corrected 17 July 2026, see P5.10.** `showBackground` (which renders the fixed `#bg` element) is only ever set on the homepage, and the homepage's featured carousel's first slide also carried `loading="eager"` (the shortcode's `imageShortcode()` sets `fetchpriority="high"` automatically whenever `loading="eager"`) — genuinely two competing high-priority images on one template. Reasoning from each element's rendered *size* at a 390×844 viewport (`#bgImg` spans effectively the full viewport; the carousel slide is much smaller and only partially in view), the 16 July pass concluded `#bgImg` was the real LCP element and dropped the carousel's eager load to a flat `"lazy"`.

Running actual Lighthouse traces during P5.10 (17 July 2026) showed this reasoning was wrong: Chrome's real LCP algorithm identifies the **carousel's first-slide image**, not the fixed background, as the homepage's actual LCP element — visible size alone doesn't determine LCP candidacy, and a `position: fixed` background layer apparently doesn't compete for it the way plain in-flow content does. Lighthouse's own `lcp-discovery-insight` audit flagged the lazy-loaded carousel image directly: `fetchpriority=high should be applied` and `LCP resources should not use loading=lazy`, both failing. Reverted the 16 July change back to `("eager" if loop.first else "lazy")`. This is the corrected, measurement-backed state — trust the Lighthouse LCP element identification over geometric reasoning about "which element looks bigger," which is exactly the mistake made here. See P5.10 for the before/after numbers this produced.

Font loading was already addressed by P5.1 — only two small (37–48KB) files are preloaded, well within budget to avoid becoming a competing bottleneck.
...
- **Unique core-page metadata** — already done in P3.8.
- **A real 1200×630 share asset, with OG image width/height/alt.** The raw hero/cover photos this site uses are neither that size nor that aspect ratio, and P5.2 already ruled out excluding them from the build output because `og:image`/`twitter:image` resolve to those exact raw files. Rather than requiring new cropped photography (which would need a business decision this pass didn't have), added a build-time step that generates a real 1200×630 cover-fit crop of whichever source image applies to each page (the project's `heroImage`, the service's `coverImage`, or the site-wide default `shareImage`) via `sharp`, cached by a hash of the source path in the same generated-assets dir the `{% image %}` shortcode already uses. `og:image` now points at this generated crop, and `og:image:width`/`og:image:height` are hardcoded to `1200`/`630` since the crop guarantees that size by construction. `og:image:alt`/`twitter:image:alt` were already present from P2.5.

  Getting this right took a real detour worth recording: the first attempt called the generator as an async Nunjucks shortcode from inside a `{% set %}...{% endset %}` capture block in `base.njk`. That produced no build error but silently rendered an empty string for `og:image` on every page — a variant of the exact async/`{% set %}` fragility already documented in P2.3's card-consolidation postmortem, just outside a loop this time. Rebuilt on a different, more robust architecture instead: all needed share-image crops are generated up front in an `eleventy.before` hook (scanning `settings.json`, every project's `heroImage`, and every service's `coverImage` directly via `gray-matter`, before any template renders), stored in a plain object, and exposed to templates only as a synchronous `shareImageUrl` filter — no async work happens inside template rendering at all. Verified by reading every generated file's actual dimensions with `sharp` (confirmed all 13 generated crops are exactly 1200×630) and by extracting and `JSON.parse`-ing the rendered `og:image*` meta tags on a project, a service, and the homepage.
- **`ProfessionalService` and service structured data, using only truthful fields.** Extended the homepage's existing `Organization` JSON-LD to also carry `@type: "ProfessionalService"`, `email`, `description`, a `PostalAddress` (locality/country only — `London`/`GB`, both already-published facts, no street address invented), `areaServed`, and a `hasOfferCatalog` listing all six published services (name + `metaDescription`/summary, reusing the P3.8 copy verbatim). Deliberately omitted `telephone` and `priceRange` — neither is published anywhere, and the enquiry budget ranges are still an open business decision per `docs/overhaul/DECISIONS.md`, so inventing a `priceRange` would violate the plan's own "don't claim what isn't substantiated" rule. No `AggregateRating`/review data either, since no genuine testimonial content exists.
- **Breadcrumb structured data.** Added `BreadcrumbList` JSON-LD to project and service detail pages (`Home > Work > {Project}` / `Home > Services > {Service}`), matching the site's real, live URL hierarchy rather than an invented one.
- **Sitemap `lastmod`, sourced from dependable content dates.** Added a `lastmod` filter that reads each content file's actual last-commit date via `git log -1 --format=%cI`, not filesystem mtime (which a fresh checkout resets to checkout time, discarding the real edit history) — falls back to build time only if git has no history for a file (e.g. new uncommitted content), so a missing/shallow git history can never break the build. Wired into every sitemap entry: static pages map to their content JSON file, legal/app pages to their shared data file, and projects/services to their own markdown file via `item.inputPath`. Verified live: distinct files show distinct, correct real commit timestamps (confirmed the timezone-offset format `git log` produces is unambiguously different from the plain build-time fallback's UTC `Z` format, so there's no risk of misreading one as the other).
- **Canonical/robots/sitemap/redirect agreement.** Wrote a verification script cross-referencing every generated page's `noindex` meta state, canonical URL, and sitemap membership: zero mismatches (nothing `noindex` is in the sitemap; nothing indexable is missing from it). `robots.txt` allows crawling and references the sitemap; admin pages rely on their `noindex` meta tag rather than a `robots.txt` disallow, which is the correct approach (blocking crawl entirely can prevent a search engine from ever seeing the `noindex` directive). Redirects were already fully validated during P3.9.
- **Thin/incomplete pages out of search** — already enforced (P1.10): coming-soon projects are `noindex` and excluded from the sitemap, confirmed by the same cross-reference script above.

Verified throughout with `npm run check` (full gate) and a live preview; no visual regressions.
...
**Implemented (16 July 2026), split by confidence level.** All added via `netlify.toml` header rules.

- **`X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy` (blocking camera/microphone/geolocation/payment/usb, plus `interest-cohort=()`), `X-Frame-Options: DENY`, and `Strict-Transport-Security: max-age=31536000; includeSubDomains`** — applied identically everywhere via a general `/*` rule, admin included; none of these differ between public and admin, and Decap's file-based image upload doesn't touch the in-page camera/mic APIs `Permissions-Policy` restricts. HSTS deliberately omits `preload`: submitting to browsers' preload lists is a separate, hard-to-reverse, cross-site decision that belongs to the site owner, not something to opt into unasked.
- **Public-page CSP — fully verified, not just written.** `default-src 'self'` plus specific allowances (`img-src` adds `data:` for one inline-SVG background image in the compiled stylesheet; everything else stays `'self'`). This was possible because the public site's entire resource graph is already self-contained: no analytics/tracking/chat/ad script anywhere, fonts self-hosted (P5.1), and — found and fixed as part of this ticket — one remaining inline `style` attribute (a hardcoded `text-align: center` on one service page's CTA band) that would otherwise have forced a `style-src 'unsafe-inline'` weakening; replaced with the site's existing `.text-center` utility class instead, so `style-src 'self'` needs no exception at all. Verified by temporarily adding this exact policy as a `<meta http-equiv>` tag in `base.njk`, rebuilding, and checking the browser console for violations across every page type and interaction that exists — homepage, Work index, a project page (including the image-gallery carousel), a service page, Contact (including an actual form submission attempt), the mobile nav menu, Studio, and a legal page. Zero CSP violations anywhere; the only console errors seen were the already-expected "501" from the local static-file preview server not supporting POST (a real HTTP response reaching the server, proving `connect-src 'self'` was never the blocker) — not a CSP block. Removed the temporary meta tag afterward; the header in `netlify.toml` is the real, permanent mechanism.
- **Admin-page CSP — a scoped starting point, explicitly not verified against the live CMS, flagged for that as separate work.** Decap CMS's actual runtime needs (its `unpkg.com`-hosted bundle, the GitHub-backed OAuth flow via Netlify's proxy per `admin/config.yml`'s `backend: name: github`) can't be exercised end-to-end from this environment — that needs a real login/edit/save/publish round trip, which is P5.7's job, not this ticket's. Wrote a CSP scoped to what the static config actually declares Decap needs (`script-src` allows `unpkg.com`; `connect-src` allows `api.github.com` and `api.netlify.com`) rather than leaving it wide open, but this is a best-effort starting point, not a finished hardening pass — expect to loosen or tighten specific directives once real CSP violation reports come back from actually using the CMS under it.

Did not pursue report-only mode for either policy: the public policy is fully verified as described above, and the admin policy's real risk is better addressed by testing the actual login/publish flow directly (P5.7) than by adding a report-only intermediate step that would just defer the same verification work.
...
**Implemented and partially verified (17 July 2026).**

- **Integrity protection for the unpkg-hosted admin script.** `admin/index.html`'s Decap script tag already pinned an exact version (`decap-cms@3.6.3`) but had no integrity check — a compromised or mistakenly-swapped file at that same URL would have run without any warning. Downloaded the exact pinned file, computed its SHA-384 digest, and added `integrity`/`crossorigin="anonymous"` attributes; the browser now refuses to execute the script at all if the bytes ever stop matching. Verified live: loaded `/admin/` in a preview and confirmed via console logs (`decap-cms-app 3.6.3`, `decap-cms-core 3.6.2`, `decap-cms 3.6.3`) that the script still loads and initialises correctly with the hash in place — a wrong hash would have blocked it outright, not just logged a warning, so this is a real pass/fail test, not an assumption.
- **Admin pages stay `noindex`** — already true, unchanged.
- **Editorial workflow enabled — a real, load-bearing gap, not just a missing nice-to-have.** `admin/config.yml` had no `publish_mode` set, meaning every CMS save committed directly to `main` with no review step at all — directly contradicting the reasoning `docs/overhaul/DECISIONS.md` gives for keeping Decap as primary CMS ("gives a deploy-preview safety net before anything publishes"). That safety net didn't actually exist until now. Added `publish_mode: editorial_workflow`, which moves saves to their own Draft → In Review → Ready states on a separate branch/PR (with its own Netlify deploy preview) instead of publishing straight to production. **This is a real change to how content gets published going forward** — the site owner will need to review and merge a pull request (or move an entry to "Ready" in Decap's own workflow UI) rather than having every save go live immediately. Verified the setting doesn't break anything: reloaded the local CMS backend after adding it, confirmed all four collections (Settings, Pages, Services, Projects) still list and load their entries normally with no console errors.
- **Schema generation from a shared content contract** — not pursued. `admin/config.yml` remains hand-maintained YAML; a true generator (deriving the CMS schema and the public templates' expectations from one shared source) would be a substantial rearchitecture disproportionate to this pass, and the existing `tools/check-content.js` contract check already catches drift between the two without needing that rearchitecture. Left as a real, larger future improvement rather than forced into this ticket.
- **Live-tested a real save round trip, with an honest account of what that testing surfaced.** Started `npm run cms:local` and exercised the actual local-backend-authenticated CMS in a browser: logged in, browsed all four collections, opened the Site Settings entry, edited the Location field, and published — confirmed with a direct file read that `src/content/settings.json` was genuinely rewritten on disk, not just updated in the browser. Reverting the test value through the CMS UI a second time proved unexpectedly unreliable — Decap's own local autosave/draft state and its actual backend-publish state look identical in the UI (both show "Published"/"Changes saved"), so it wasn't obvious from the interface alone whether a given edit had actually reached the file or only a local draft cache; a raw coordinate-based click also turned out to land on the wrong control more than once in this environment (ref-based clicks were reliable, raw screenshot coordinates were not). Reverted the test value directly with a file edit instead of continuing to fight the CMS UI, confirmed byte-identical against the last commit, and stopped there rather than keep guessing at a UI interaction pattern under time pressure.
- **Not tested: create/unpublish/archive/image-upload, or the editorial workflow's actual PR-creation behaviour against real GitHub.** The settings-field edit above proves the core local-backend read/write mechanism works; it doesn't prove every collection type or the full draft lifecycle. The `editorial_workflow` setting was confirmed not to break anything, but its actual PR-per-entry behaviour only manifests against the real `github` backend (Netlify + GitHub OAuth), which can't be exercised from this environment. **Owner action needed:** do at least one real create → edit → publish → merge round trip against the live production CMS before relying on this as "hardened."
...
**Attempted (17 July 2026), with an important caveat: measured against a local static-file server, not a production deploy.** The plan's own instruction is "production-like builds" — a bare `python3 -m http.server` serving `_site/` is a real production build's *output*, but HTTP/1.1 only, no HTTP/2 multiplexing, no CDN edge, no TLS — all things that affect real-world Lighthouse timing, especially for a page loading multiple parallel resources (fonts, background image, carousel images) where HTTP/1.1's per-origin connection limit can serialise requests a real CDN wouldn't. Treat the numbers below as a directional signal that surfaced real, fixable issues (and it did) — not as the trustworthy production measurement the plan asks for. **Re-running this against the actual Netlify production deploy is real remaining work, not yet done.**

Ran Lighthouse's default mobile profile (412×823, DPR 1.75, throttled) 3× against the homepage via `npx lighthouse`, median of three:

| Metric | Before this pass | After fixes | Target | Met? |
| --- | ---: | ---: | ---: | --- |
| Performance | 82 | **89** | ≥85 | ✅ |
| Accessibility | 100 | 100 | 100 | ✅ |
| Best Practices | 100 | 100* | 100 | ✅ |
| SEO | 100 | 100 | 100 | ✅ |
| LCP | ~4056ms | **~3477ms** | ≤2500ms | ❌ (improved, still over) |
| TBT | 202ms | **0ms** | ≤200ms | ✅ |
| CLS | 0 | 0 | ≤0.1 | ✅ |
| Total transfer | 577KB | **491KB** | ~400KB | ❌ (improved, still over) |

\* One of the three runs scored Best Practices 96 on a transient `errors-in-console` finding (a 404 for a generated image derivative that demonstrably exists in `_site/img/` — confirmed by listing the directory directly). This reproduced in only one of three identical runs against an unchanged build, matching the exact "local static-file-server flakiness under simulated throttling" pattern this repo's own visual-reference tooling already documents elsewhere (P2.2) — not a real site defect.

**A real bug found and fixed in the process, superseding P5.4's 16 July conclusion.** Lighthouse's `lcp-discovery-insight` audit identified the homepage's actual LCP element as the featured carousel's first slide, not the fixed background `#bgImg` — directly contradicting the geometric "which element is bigger" reasoning P5.4 used the day before. Reverted P5.4's change (restored `loading="eager"` to the carousel's first slide); see P5.4 above for the full account. This alone didn't move LCP much (~4056ms → ~4057ms unchanged) — the real win came from the second finding below.

**The real win: the background image's own responsive tiers had a gap at exactly Lighthouse's mobile profile.** `network-requests` showed the homepage's single biggest transfer was `background-1280.avif` at 141KB. Lighthouse's mobile emulation (412px × 1.75 DPR) needs a 721px-effective-width image; the existing tiers (`640, 1280, 1920, 2560`, added in P5.2) jump straight from 640 (insufficient) to 1280 (2.7× the pixel area actually needed), forcing the far-larger file. Generated a `960`-wide intermediate tier (avif + webp, via `sharp` from the `2560` source) and added it to `base.njk`'s `<picture>` element, mirroring the exact process used for the `640` tier in P5.2. This dropped the same request to a much smaller file and was the single largest contributor to the performance-score and LCP improvement above.

**What's still over budget, and why it wasn't chased further this pass:** LCP (~3477ms, still ~1s over target) and total transfer (~491KB, ~90KB over target). Both improved substantially but didn't fully close given further gains would mean either accepting an image-quality tradeoff (reducing AVIF/WebP quality settings further, or the background image's own resolution) — a real visual-quality decision, not a mechanical fix — or a deeper architectural change (e.g., critical-CSS extraction, already considered and set aside in P5.3 as disproportionate to this stylesheet's actual size). Flagging rather than guessing at a quality tradeoff the site owner hasn't been asked about.

**Also spot-checked (single run, not median-of-three) the published project page** — Performance 90, Accessibility/Best Practices/SEO 100, LCP 2855ms, TBT 254ms, CLS 0.018, transfer 293KB (under the 400KB budget). Directionally healthy, not exhaustively verified to the same median-of-three standard as the homepage.

**Not done:** the redundant, more thorough sweep across every other template (Services index, Studio, Contact, a service detail page, legal pages) at median-of-three, and — most importantly — re-running all of this against the real Netlify deploy once one exists, which is the only way to get a number this ticket's own "production-like builds" instruction would actually trust.
...
- [x] Responsive-image selection is verified at target viewport sizes (P5.2, 16 July 2026) — the specific known gaps (the 400→800 width jump, the carousel's padding-mismatched `sizes`, and the oversized mobile background) are fixed and spot-verified at 375px/2x DPR in a fresh browser tab; this wasn't an exhaustive per-template sweep of every image on the site.
- [ ] Median performance and accessibility budgets pass. Measured locally (P5.10, 17 July 2026) against a static-file server, not production: performance (89), accessibility/best-practices/SEO (100/100/100), and TBT/CLS all now pass; LCP (~3477ms) and total transfer (~491KB) improved substantially but remain over budget. Not checked off because this needs re-measuring against the real Netlify deploy — a local server isn't "production-like" in the way this ticket requires.
- [x] Structured data validates and contains only truthful content (P5.5, 16 July 2026) — `Organization`/`ProfessionalService`, `BreadcrumbList`, and per-service `Offer`/`Service` JSON-LD all parse as valid JSON and use only already-published facts; no `priceRange`, `telephone`, or rating data invented.
- [x] Redirects, canonicals, robots, and sitemap agree (P3.9 and P5.5, 16 July 2026) — a cross-reference script confirms zero mismatches between each page's `noindex` state, canonical URL, and sitemap membership; the full old→new redirect map was verified during P3.9.
- [ ] Security headers work on public and admin routes. Public-page CSP and the other headers (P5.6, 16 July 2026) are fully verified against every page type and interaction. The admin-page CSP is only a scoped starting point — it still needs a real Decap login/edit/save/publish round trip against it (P5.7) before this can be checked off.
- [ ] The chosen CMS passes a full content round-trip. Partially verified (P5.7, 17 July 2026): a real settings-field edit → publish → disk-write round trip was confirmed against the local backend. Not yet tested: create/unpublish/archive/image-upload, and the new editorial-workflow PR behaviour against the real GitHub backend — needs a live owner test before this can be checked off.
...
As of July 2026, groups 1–5 are closed (Phases 1 and 2 both have fully-checked exit gates; see the status summary in §1). Group 6 (Phase 3) is well advanced: P3.1, P3.2, P3.3, P3.4, P3.6, and P3.8 have landed; P3.5 was declined by the site owner — see the P3.5 section and `docs/overhaul/DECISIONS.md` — and should not be re-attempted without being explicitly asked. P3.7 is resolved: the Garden Restaurant image fix landed; the six service cover images have clean twins in the repo but the site owner chose to keep their baked text (recorded decision, not a gap); Marylebone Terrace's baked-text hero was explicitly deprioritized and isn't tracked as open work. P3.9 is now fully implemented — every public URL except `/apps/*` and the homepage moved to a clean directory URL, with a complete redirect map. Group 7 (P4.1–P4.9) started 17 July 2026. P4.1 (global nav/footer): an initial pass added a Services link to the primary nav/footer, which caused a real layout regression (overlapping the homepage's own pill row at 1280–1480px) and, independent of that bug, was reverted outright at the site owner's request — Services is deliberately not in the primary nav/footer, same standing-decision status as the P3.5 homepage layout. P4.3 (work index), P4.4 (project case study), and P4.7 (contact/forms) are done. P4.2 (homepage) needed no changes. P4.8 (supporting pages) is done. P4.9 (responsive/input-mode review) is done except real-device spot checks, which need the site owner's own hardware. P4.5 (services) is partially done: its breadcrumb is built, but the site owner opted to keep the service cover images' baked text rather than swap to the clean versions that turned out to already exist (P3.7). P4.6 (studio) is done — a real Founder section now introduces the site owner by name and title, with facts gathered directly from him via `docs/overhaul/STUDIO-BIO-INTERVIEW-BRIEF.md`; only portrait/environmental photography remains absent, which he confirmed isn't ready yet. Group 8 (P5.1–P5.4) and most of group 9 (P5.5–P5.7) are also done, ahead of group 7 — none of that work depended on Phase 4's page-by-page UI pass. P5.8 is moot (P0.3); P5.9 needs a legal professional; P5.10 needs a production-like deploy to measure meaningfully and hasn't been attempted.
