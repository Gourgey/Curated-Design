...
**Implementation note (17 July 2026, partial):** the Studio page had a full "Documents & tools" section — its own eyebrow, heading, and a four-item hairline list of legal/app links styled at the same visual weight as the rest of the editorial body. Removed it entirely (`about.njk`, plus the now-dead `about.documents` CMS field and `.studio-docs`/`.doc-list` CSS) rather than just shrinking it: the footer already carries the same Legal and App-support links, generated from the live `legal.pages`/`apps.items` collections rather than this hand-maintained duplicate, so removing it also closes a content-drift risk, not just a visual-weight one. **Not done, and can't be done without the site owner's input:** the ticket's core ask — "introduce the person or studio behind the work with approved imagery and facts." There is currently no named individual, biography, or portrait/environmental photo anywhere on the Studio page or in its content data (`src/content/pages/about.json`) — only philosophy, process, and location copy. This needs real biographical facts and an approved photo from the site owner before it can be written; it isn't something to draft or invent. This ticket stays open until that content exists.
...
- [ ] Completed work and studio credibility appear before speculative content. Completed-work ordering is done (P3.3/P4.3); studio credibility is not — P4.6 still needs real biographical facts and an approved photo from the site owner.
