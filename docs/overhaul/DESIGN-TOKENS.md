...
| 900px | Project-detail two-column article (`.project-article`) and service two-column body (`.service-body`, `.service-body__aside`) | **Keep — distinct, correctly tuned, and deliberately synced with the nav's 900px/901px pair above.** Both pages use the same pill-nav compact toggle, so their content reflow lines up with it rather than the shared 920px. | Missed by the original audit (it only covered the nav's own 900/901 pair, not these two separate uses of plain `900px`). Checked live at the 900px boundary: `.project-article`'s aside column is 315px wide, `.service-body__aside`'s is 309px — both comfortably fit their content (a facts list / enquiry form), not cramped. |

The lesson generalises: a breakpoint value that doesn't match the shared set is not automatically legacy debt. Four of these five turned out to already be correctly tuned for their specific component and would have been made *worse* by forcing them onto a shared number. Only change a breakpoint's value when you've captured and looked at the actual gap it changes — never from the 390/768/1440px baseline alone.
...
| `04-homepage.css` | Hero content, section bands. (Carousel mechanics moved out — see `12-carousels.css`; the shared CTA button/band moved out — see `13-buttons-cta.css`; the thank-you page moved out — see `07-misc-pages.css`.) |
| `05-legacy-pages.css` | Early Projects/About/Contact/Concept-Design page rules, plus interspersed floating-nav-overlay readability fixes. (The contact form's shared field styles moved out — see `14-form-fields.css`.) |
| `06-project-template.css` | The project detail template's original (pre-redesign) rules: hero text content (kicker/title/subtitle/tags), topbar. (Hero carousel mechanics moved out — see `12-carousels.css`; the backlink moved out — see `15-backlink.css`.) |
| `07-misc-pages.css` | Coming-soon badge, Apps pages, Studio legal pages, the thank-you page (`.thanks`, `.thanks__inner`, moved here from `04-homepage.css` — not homepage-specific). (The apps/legal backlinks moved out — see `15-backlink.css`.) |
| `08-homepage-redesign.css` | The homepage redesign pass — Featured Projects, Design Philosophy, shared section spine. (The homepage services grid's card component moved out — see `16-card.css`.) |
...
| `10-projects-listing-redesign.css` | "Projects Listing — redesign 2026" — supersedes parts of `05-legacy-pages.css`. (The project card component moved out — see `16-card.css`.) |
...
| `13-buttons-cta.css` | **By-topic regrouping** — the shared CTA button/band component (`.button`, `.about-cta-lead`, `.about-cta-button`, `.about-cta-band`, `.cta-band__inner`), gathered from `04-homepage.css`. Backs `src/_includes/partials/cta-band.njk`, rendered on the homepage, projects listing, service, and project-detail pages — not homepage-specific despite where it used to live. |
| `14-form-fields.css` | **By-topic regrouping** — the shared enquiry field component (`.field`, `.field__label`, `.field__input`, `.field__select`, `.field__textarea`, `.field--optional`) used by both `.contact-form` (contact page) and `.service-enquiry__form` (service pages), gathered from `05-legacy-pages.css` and `11-service-redesign.css`. The two forms render the identical structure at two different scales; per-scope CSS custom properties (`--field-value-size`, `--field-textarea-min-height`, etc.) preserve each form's exact sizing. The submit row/button and privacy/error/success text stayed behind in each page's own partial — those differ in real structural ways, not just scale. |
| `15-backlink.css` | **By-topic regrouping** — the shared "back to X" link component (`.backlink`, `.backlink-row`, `.backlink--pill`, `.backlink--hero`), gathered from `06-project-template.css` and `07-misc-pages.css`. `.backlink--pill` covers the apps and studio legal pages' back-links, which were byte-for-byte identical rules under two different class names (`.app-backlink`, `.legal-backlink`); `.backlink--hero` is the project detail page's own italic-serif treatment over the dark hero image. See `docs/overhaul/COMPONENTS.md` for the consolidation decision. |
| `16-card.css` | **By-topic regrouping** — the shared project/service card component (`.card`, `.card--project`, `.card--service`, `.card__media`, `.card__title`, `.card__kicker`, `.card__summary`), gathered from `08-homepage-redesign.css` and `10-projects-listing-redesign.css`. The wrapper/media/hover shape and title styling (parameterised via `--card-title-size`: 22px project, 24px service) were identical; `.card__kicker` (a taxonomy label) and `.card__summary` (a description sentence) stayed as separate classes since they're genuinely different content, not the same field renamed. See `docs/overhaul/COMPONENTS.md`. |
