...
**Status (July 2026): duplicate-selector reconciliation, breakpoint consolidation, the physical (order-preserving) file split, and legacy token-alias retirement are complete. True by-topic regrouping is not.**
...
**Not done:** true by-topic regrouping (e.g. gathering every carousel rule — currently split between the homepage partial and the two project-template partials — into one file) and reducing `body.page`-specificity reliance. This requires moving rules relative to each other, which needs the same one-change-at-a-time, pixel-diff-verified treatment as the duplicate-selector work above, at the scale of the whole file rather than 27 known selectors.
